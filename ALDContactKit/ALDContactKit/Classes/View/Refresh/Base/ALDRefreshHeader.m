...
@interface ALDRefreshHeader()

@end

@implementation ALDRefreshHeader
#pragma mark - 构造方法
+ (instancetype)headerWithRefreshingBlock:(ALDRefreshComponentRefreshingBlock)refreshingBlock
{
    ALDRefreshHeader *cmp = [[self alloc] init];
...
- (void)prepare
{
    ...
    // 设置key
    self.lastUpdatedTimeKey = ALDRefreshHeaderLastUpdatedTimeKey;
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change
{
    ...
    // 在刷新的refreshing状态
    if (self.state == ALDRefreshStateRefreshing) {
        // sectionheader停留解决
        return;
    }
    ...
    if (self.scrollView.isDragging) { // 如果正在拖拽
        ...
        if (self.state == ALDRefreshStateIdle && offsetY < normal2pullingOffsetY) {
            // 转为即将刷新状态
            self.state = ALDRefreshStatePulling;
        } else if (self.state == ALDRefreshStatePulling && offsetY >= normal2pullingOffsetY) {
            // 转为普通状态
...
- (void)setState:(ALDRefreshState)state
{
    ...
    if (state == ALDRefreshStateIdle) {
        if (oldState != ALDRefreshStateRefreshing) return;
