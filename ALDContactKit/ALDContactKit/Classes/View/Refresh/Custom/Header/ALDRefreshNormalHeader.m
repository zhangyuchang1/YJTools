...
#import "ALDRefreshNormalHeader.h"

@interface ALDRefreshNormalHeader()
{
...
@implementation ALDRefreshNormalHeader
#pragma mark - 懒加载子控件
- (UIImageView *)arrowView
{
    if (!_arrowView) {
        UIImage *image = [UIImage imageNamed:ALDRefreshSrcName(@"arrow.png")];
        if (!image) {
            image = [UIImage imageNamed:ALDRefreshFrameworkSrcName(@"arrow.png")];
        }
    ...
    if (state == ALDRefreshStateIdle) {
        if (oldState == ALDRefreshStateRefreshing) {
            ...
            [UIView animateWithDuration:ALDRefreshSlowAnimationDuration animations:^{
                ...
                if (self.state != ALDRefreshStateIdle) return;
